...
# --- Google Sheets setup for persistence ---
...
# --- Ensure header row is correctly named ---
sheet.update("A1:C1", [["id", "Item", "Amount"]])

# --- Load persisted log into session state ---
if "log" not in st.session_state or "next_id" not in st.session_state:
    rows = sheet.get_all_records()
    st.session_state.log     = rows
    st.session_state.next_id = max((row["id"] for row in rows), default=0) + 1
...
# --- Base foods (units are per label) ---
base_foods = [
...
# --- Session state for custom foods & init foods_df ---
...
st.dataframe(
    ref_df.rename(columns={
        ...
        "Alc/unit":"Alcohol (g)","Calories per unit":"Calories per unit"
    })[
...
with st.form("log_form", clear_on_submit=True):
    ...
    if st.form_submit_button("Add to Log"):
        ...
        # persist to Google Sheet + in-session
        sheet.append_row([entry["id"], entry["Item"], entry["Amount"]])
...
if st.session_state.log:
    ...
    n_df.rename(columns={
        "P_g":"Protein_g","F_g":"Fat_g","C_g":"Carbs_g",
        "Fiber_g":"Fiber_g","Alc_g":"Alcohol_g"
    }, inplace=True)
    ...
    if st.button("Reset Log", key="reset_log_button"):
        # clear sheet + re-add header
        sheet.clear()
        sheet.append_row(["id", "Item", "Amount"])
